...
results['config'] = {'epochs': EPOCHS, 'num_rounds': num_rounds, 'pruning_ratio': pruning_ratio, 'num_experiments': num_experiments, 'hidden_size': hidden_size}
...
def get_multiple_models(n_models, train_loader, test_loader, train_set, test_set, hidden_size=32, silent=True):
    ...
    for i in range(n_models):
        ...
        # if not silent: print(f'Starting model {i}')
        ...
        grad_hist = train_loop(prunable, train_loader, loss_fn, optimizer, EPOCHS, silent=True)
...
for i, prunable in enumerate(models):
    ...
    # now we have found the lottery ticket
    prunable.apply_saved_initialization()
    timer = pf() - timer
    prunable.to(device='cpu')
...
for i, prunable in enumerate(models):
    cpumask = {}
    for key in prunable.mask.keys():
        cpumask[key] = prunable.mask[key].cpu().clone().detach()
...
# for i in range(len(models)):
    ...
    # print(type(prunable), id(prunable))
    train_loop(prunable, train_loader, loss_fn, new_optimizer, n_epochs, silent=True)
...
for i in range(num_experiments):
    ...
    if test_acc - results['FC-test-accs'][prunable_idx] >= 0.005:
